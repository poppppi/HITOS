...
更正：来自@jiwangyihao:实时系统应当采用基于优先级的抢占式调度，确保高优先级的进程立即执行，并在可预期的时间内完成。
...
来自@jiwangyihao：1.  **创建目标文件 `dest.txt`：**
    * (1) 为 `dest.txt` 分配一个**空闲 inode**。
    * (2) 在目录中创建 "dest.txt" 的**目录项**，指向该 inode。
2.  **循环复制数据（`read` source, `write` dest）：**
    * (3) OS 从 `source.txt` 的 inode 指针中读取数据块。
    * (4) OS 为 `dest.txt` 分配一个**空闲数据块**。
    * (5) OS 将数据写入这个空闲数据块。
    * (6) OS 将这个数据块的地址记录到 `dest.txt` 的 inode 中。
3.  **处理大文件：**
    * (7) 当 `dest.txt` 的**直接指针**用完时（比如 7 个），OS **额外分配一个“指针块”**，并让 inode 中的“**间接指针**”指向它。
    * (8) 后续的数据块地址（第8、9、10...块）被依次写入这个“指针块”中。
    * (9) （如果文件更大）当这个“间接指针块”也写满时，OS 会启用“**双重间接指针**”，去管理一个“指向指针块的指针块”。
